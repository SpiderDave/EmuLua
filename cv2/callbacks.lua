...
registerExec(0xcc30+2,7,1,"onPauseCheck")
registerExec(0xd31e+2,7,1,"onPauseCheck2")

registerExec(0xd33e,7,1,"onPause") -- pause or enter sub menu
registerExec(0xd329+2,7,1,"onUnpause") -- unpause
registerExec(0xf3fe+2,7,1,"onExitSubMenu") -- exit sub menu
...
registerExec(0xf5e2+2, 7,1,"onWindowPrintChar")
...
-- Works for some states, not all
function _onSetPlayerState(address,len,t)
    if type(onSetPlayerState)=="function" then
        local a = onSetPlayerState(t.a)
        if a then memory.setregister("a", a) end
    end
end

function _onPauseCheck(address,len,t)
    if type(onPauseCheck)=="function" then
        local a = onPauseCheck(t.a)
        if a then memory.setregister("a", a) end
    end
end

function _onPauseCheck2(address,len,t)
    if type(onPauseCheck2)=="function" then
        local buttons, pauseStatus = onPauseCheck2(t.a, t.y)
        if buttons then memory.setregister("a", buttons) end
        if pauseStatus then memory.setregister("y", pauseStatus) end
    end
end

function _onPause(address,len,t)
    if t.a == 0x02 then
        if type(onPause)=="function" then
            local a = onPause(t.a)
            if a then memory.setregister("a", a) end
        end
    elseif t.a == 0x01 then
        if type(onEnterSubMenu)=="function" then
            local a = onEnterSubMenu(t.a)
            if a then memory.setregister("a", a) end
        end
    end
end

function _onUnpause(address,len,t)
    if type(onUnpause)=="function" then
        local a = onUnpause(t.a)
        if a then
            memory.setregister("a", a)
            --memory.writebyte(0x26, a)
        end
    end
end

function _onExitSubMenu(address,len,t)
    if type(onExitSubMenu)=="function" then
        local a = onExitSubMenu(t.a)
        if a then
            memory.setregister("a", a)
            --memory.writebyte(0x26, a)
        end
