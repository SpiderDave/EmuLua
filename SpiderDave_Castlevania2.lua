...
--        memory.writebyte(0x0080, objects.player.hp)
    ...
    {tab="leftclick", text="Shop", action=function()
        config.leftClick = "shop"
    end},
...
for i,w in ipairs(game.data.warps or {}) do
    --return string.format("Warp %02x %s", game.data.warpNum or 1, cv2:getAreaName(w.area1,w.area2,w.area3))
    game.debugMenuItems[#game.debugMenuItems+1]={tab="warp", text = function() return cv2:getAreaName(w.area1,w.area2,w.area3) end, action = function()
        game.data.warpNum = i
        warpPlayer(w)
        --cv2.warp(w)
    end}
...
--gfx.savecrystal = gfx.load("savecrystal")
gfx.savecrystal = gfx.load("whitecrystal")
...
function equipItem(itemName)
    ...
    if item.type=="dagger" then setWhip(item.index) end
...
function sortItems()
    ...
    local newList = {}
    local types={"weapon", "use", "whip", "dagger", "armor", "accessory"}
...
function updateItems()
    ...
    for i=1,#itemList do
        ...
        if itemList[i].type=="dagger" then
...
function warpPlayer(w)
    w.area1 = w.area1 or area1
    w.area2 = w.area2 or area2
    w.area3 = w.area3 or area3
    w.areaFlags = w.areaFlags or areaFlags
    w.returnArea = w.returnArea or returnArea
    w.returnScroll1 = w.returnScroll1 or returnScroll1
    w.returnScroll2 = w.returnScroll2 or returnScroll2
    w.returnX = w.returnX or returnX
    w.returnY = w.returnY or returnY
    w.playerX = w.playerX or objects.player.x
    w.playerY = w.playerY or objects.player.y
    w.scrollX = w.scrollX or scrollx
    w.scrollY = w.scrollY or scrollx

    cv2.warp(w)
end

--function _warpPlayer(w)
...
--    objects.player.y=w.playerY or objects.player.y
...
function openShop(t)
    game.shop=game.shop or {}
    game.shop.data = t
    game.shop.data.message = game.shop.data.message or "Shop"
    game.shop.messageIndex = 0
    game.shop.messageSpeed = game.shop.messageSpeed or 2 -- default message speed
    memory.writebyte(0x0026,3) -- special shop pause
end
...
function onSetPlayerState(state)
    if state == 5 then
        -- make the initial drop when falling off a ledge quicker
        memory.writebyte(0x036c, 6)
    end
end
...
function onTalk(t)
    ...
    if (objects.player.state or 0) > 1 then return false end
end

-- also affects marsh damage
...
--    local base = 3
--    d = math.floor(base - stats.con * .1)
    
    --spidey.message("%d", stats.con)
--    spidey.message("%d", d)
...
function onPauseCheck2(a, y)
--    local n = memory.readbyte(0x00f5)
--    if n~=0 then
--        spidey.message("%02x", n)
--    end
    --spidey.message(n)
    --if n==0x20 then return 0 end
--    if n==0x20 then return 0 end
...
--function onEnterSubMenu(a)
--    spidey.message("onEnterSubMenu %02x", a)
--end
--function onExitSubMenu(a)
--    spidey.message("onExitSubMenu %02x", a)
--end

--function onPause(a)
--    spidey.message("onPause %02x", a)
--end

-- return 2 to remain paused instead
function onUnpause(a)
    --spidey.message("onUnpause %02x", a)
    
    if game.showSaveMenu then
        -- remain paused with special value of 4
        return 4
    end
...
function createMedusaHead()
    ...
    if not obj then
        spidey.message("Could not create object")
        return
    end
    obj.facing = facing
    obj.xs=1.17
...
--    local startIndex = game.nextUnusedIndex or 1
    
--    for i=startIndex,objects.custom.count-1 do
--        if objects.custom[i].active==0 then
--            unusedIndex = i
--            game.nextUnusedIndex = i+1
--            if game.nextUnusedIndex>objects.custom.count-1 then game.nextUnusedIndex=1 end
--            break
--        end
--    end
    
--    for i=1, startIndex do
--        if unusedIndex then break end
--        if objects.custom[i].active==0 then
--            unusedIndex = i
--            game.nextUnusedIndex = i+1
--            if game.nextUnusedIndex>objects.custom.count-1 then game.nextUnusedIndex=1 end
...
function setHp(hp)
    hp = math.min(getMaxHp(), hp or getHp())
    objects.player.hp = hp
    
    -- constrain real HP to normal limits
    memory.writebyte(0x0080, math.min(0x60, hp))
end


function setMaxHp(maxHp)
    maxHp = maxHp or getMaxHp()
    objects.player.maxHp = maxHp
    
    if getHp() > maxHp then
        setHp(maxHp)
    end
    
    -- constrain real max HP to normal limits
    memory.writebyte(0x0081, math.min(0x60, maxHp))
end

function getHp()
    -- The game handles hp for now until we can
    -- override every single hp modification and
    -- use our custom system.
    return memory.readbyte(0x0080)
    
    --return objects.player.hp or memory.readbyte(0x0080)
end

function getMaxHp()
    -- new hp formula
    
    local maxHp = 0x30+1*objects.player.level+4*relics.nParts
    --spidey.message(maxHp)
...
memory.registerexec(0xc127,1, function()
    ...
    if a == 0x0e and items[objects.player.whipItem] and items[objects.player.whipItem].type == "dagger" then a=0x11 end
...
--    memory.writebyte(0x0101, pattern1)
    ...
    if game.showSaveMenu then
        ...
        if joy[1].confirm_press then
            ...
            if memory.readbyte(0x0026) == 0x04 then
                cv2.unPause()
            end
        end
    end
    ...
    if game.mode==0x04 then
        ...
        --if game.modeCursor == 1 then
            ...
            if s=="empty" then
                ...
                local displayarea
                if areaFlags == 1 then
                    displayarea = cv2:getAreaName(0,s.returnArea,0)
                else
                    --displayarea = cv2:getAreaName(s.area1,s.area2,s.area3)
                    displayarea = cv2:getAreaName(s.area1,s.area2,s.area3,s.returnArea)
                end
...
--        if subScreen.cursorY == 4 and subScreen.cursorX > 1 then subScreen.cursorX = 1 end
    ...
    if action and items[objects.player.whipItem].type == "dagger" then
        memory.writebyte(0x0434,0x0d) -- whip
        local whipDelay = memory.readbyte(0x0457)
        local attackSpeed = items[objects.player.whipItem].attackSpeed or 4
        whipDelay = math.min(whipDelay, attackSpeed)
        ...
        local daggerLen = items[objects.player.whipItem].knifeLength or 12
        
        if whipframe==0x02 and attackSpeed > 4 then
            gui.drawline(objects.player.x-10*facing, objects.player.y-9+yo,objects.player.x-20*facing, objects.player.y-12+yo,"white")
        end
        
        if whipframe==0x03 then
            gui.drawbox(objects.player.x+12*facing, objects.player.y-2+yo, objects.player.x+(12+daggerLen-2)*facing, objects.player.y-2+2+yo, "white", "grey")
            gui.drawbox(objects.player.x+12*facing, objects.player.y-2+yo, objects.player.x+(12+daggerLen)*facing, objects.player.y-2+2+yo, "white", "clear")
...
--            interval=100
        ...
        if objects[i].name == "Orb" then
            for _,v in ipairs(cv2data.mansions) do
                if cv2:getAreaName(area1, area2, area3) == v.name then
...
--        gui.drawbox(x, y, x+16, y+16, "clear", "blue")
    ...
    if inp.leftbutton_press and config.leftClick=="shop" then
        game.shopMenu=not game.shopMenu
        if game.shopMenu then 
            openShop{
                _message = "Invest in an oak stake?",
    ...
    for i=0,objects.custom.count-1 do
        ...
        if objects.custom[i].active==1 and objects.custom.isOnScreen(i) then
            ...
            elseif objects.custom[i].type=="item" then
                ...
                if item.name=="Wall Chicken" then
                    objects.custom[i].gfx = gfx.wallchicken
                    xo=-8
                    yo=-8
                end
                if item.name=="Save Crystal" then
                    objects.custom[i].gfx = gfx.savecrystal
                    xo=-8
                    yo=-8
                end
                ...
                elseif item.gfx == "none" then
                    -- explicit lack of gfx
                else
                ...
                if objects.custom[i].xdist <= 12 and objects.custom[i].ydist <= 12 then
                    ...
                    if item.interact then
                        -- item is interactable like a sign
                        if joy[1].confirm_press then
                            interactItem(item)
                        end
                    else
...
--    end
    ...
    if game.shopMenu then
        if memory.readbyte(0x0026) ~=0x03 then game.shopMenu=false end
        local x=-4
        local y=0
        local h = 14+2+1+1
        local w = 14+10+2
        ...
        local shopItems = game.shop.data.items
        
        if game.shop.messageIndex<#game.shop.data.message then
            game.shop.messageTimer = (game.shop.messageTimer or game.shop.messageSpeed) - 1
            if game.shop.messageTimer==0 then
                game.shop.messageTimer = game.shop.messageSpeed
                game.shop.messageIndex = game.shop.messageIndex +1
                playSound(0x05)
            end
        else
            for i=1,#shopItems do
                drawfont(x+28+20,y+28+24*i,font[subScreenFont], shopItems[i])
                drawfont(x+28+32,y+28+24*i+8,font[subScreenFont], "100 Gold")
            end
            
            gfx.draw(8*3,5+8*6+msgChoice*24,gfx.arrowcursorRight)

            if joy[1].up_press_repeat and msgChoice>0 then
                msgChoice = msgChoice - 1
                memory.writebyte(0x7000+4, msgChoice)
                --playSound(0x31) -- cursor
                playSound(0x05)
            elseif joy[1].down_press_repeat and msgChoice <#shopItems-1 then
                msgChoice= msgChoice + 1
                memory.writebyte(0x7000+4, msgChoice)
                --playSound(0x31) -- cursor
                playSound(0x05)
        ...
        drawfont(x+28,y+28,font[subScreenFont], string.sub(game.shop.data.message, 1, game.shop.messageIndex))
    end
...
function spidey.draw()
    ...
    if pausemenu and game.map.visible then
        ...
        local mapLabel = function(x,y,n)
            ...
            if n==cv2:getAreaName(area1,area2,area3,areaFlags) and spidey.counter%6<3 then
    ...
    if action then
        ...
        if area1==0x02 and area2==0x09 and area3==0x02 and scrollx >= 0x265 then
            -- make invisible stairs visible if the eye is on
            if relics.on.eye then
